...
function read_int64(state: ReaderState, unsigned = false) {
  ...
  const result = unsigned
    ...
    : dataView.getBigInt64(offset, true);
  state.offset += 8;
  return result;
}
...
function read_kbidevice(state: ReaderState) {
  ...
  if (input_count === undefined) return undefined;
  const name = read_string(state);
  if (name === undefined) return undefined;
  const device_id = read_string(state);
  if (device_id === undefined) return undefined;
...
function parse_kbi_legacy_v3(state: ReaderState) {
  ...
  for (const input of inputs) {
    ...
    if (vk === undefined) console.log(`Unrecognized key: ${input.input.key}`);
    inputArr.push({
...
export function parse_kbi_legacy(buffer: Uint8Array) {
  const header = 'KBI\0';
  for (let i = 0; i < 4; ++i) if (buffer[i] != header.charCodeAt(i)) return undefined;
  const state = {
    buf: buffer,
    offset: 4,
  };
