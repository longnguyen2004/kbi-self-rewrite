...
<script lang="ts">
  ...
  // x is simply the index i, so we reuse a single points buffer and only
  // overwrite y-values in place, avoiding per-render allocation of {x, y}
  // objects.
  ...
  function render() {
    ...
    // Grid lines: only rebuild when the tick values actually changed.
    ...
    // Reuse the points buffer: x is simply the index, so we only overwrite y
    // in place. This avoids allocating a new object array on every render
    // (critical for real-time insertion).
    const n = data.length;
    if (points.length !== n) {
      points = data.map((el, i) => ({ x: i, y: el }));
    } else {
