...
export class QuantizedTickLinearScale extends LinearScale {
  ...
  public buildTicks(): Tick[] {
    ...
    if (!stepSize) throw new Error('Step size must be defined for quantized tick axis to work');
    const { min, max } = this;
    ...
    for (let i = quantizedMin; i <= quantizedMax; i += stepSize) ticks.push({ value: i });
