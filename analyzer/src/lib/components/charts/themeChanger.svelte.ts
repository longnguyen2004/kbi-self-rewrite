...
export const ThemeChanger = {
  ...
  beforeUpdate(chart, args, options: ThemeChangerOptions) {
    ...
    if (options.backgroundColor) chart.options.backgroundColor = options.backgroundColor[theme];
    if (options.borderColor) chart.options.borderColor = options.borderColor[theme];
    if (options.color) chart.options.color = options.color?.[theme];

    if (options.scales && chart.options.scales) {
      for (const id in chart.options.scales) {
        if (!chart.options.scales[id] || !options.scales[id]) continue;
